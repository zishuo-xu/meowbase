...
| `@墨墨` 与 `@团团` 各占一行 | 同题群发、顺序执行,同一正文发给所有行首目标;一只跑完再跑下一只;同一行第二个 `@` 不算。多 `@` 时第一个交棒留槽,后一个入队,当前棒跑完平台接着跑 |
| 行首 `@all` / `@thread` / `@全员` / `@大家` | 展开成名册全部,顺序执行。`@架构` / `@审查` / `@执行` 按角色展开。句中不算。猫写的群组名不交接 |
...
| `#ev_xxx` | 引用已确认证据注入当轮上下文。**不受仓库划界限制** —— 人点名 id 就是显式跨界,想借别的项目那条决定就借得到;划界只管上面那条不用手打的自动召回。某只猫没有可 resume 的 session 时,本线程已确认证据会当续接胶囊自动带上(最多 8 条)。Hub「记忆」页可搜全部已确认证据,跨仓命中标「别的项目」,点名才进提示词 |
...
| 命令白名单 | 只跑猫 `等跑` 里白名单形状;元字符拒、不在表里拒。白名单位之后只许 `--` 和 flag,中文说明当多余参数拒。命令字符串来自猫的回复 |
...
- 测试:`pnpm test`(shared 202 + api 333 + web 187 = 722);api 那 333 里有 12 个是 Redis 套件,本地 Redis 在跑才 passed,连不上则 `describe.skipIf` 真跳过(输出 321 passed + 12 skipped,总数仍是 333)。**报告数不等于 `it(` 声明数**——有参数化生成的用例,别按 grep 结果改这里
...
- **查一条线程都发生过什么**:`curl "localhost:3200/api/audit?threadId=t_xxx"`(倒序;可加 `actor` / `action` / `since` / `limit`)。球停在哪一跳、谁交给谁、哪张卡被批过、重启后哪一棒被强抢重跑,都在这里,不用翻终端。某一跳的 CLI 原始行:`GET /api/hops/:hopId?threadId=`
