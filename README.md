...
>
> **现状**:M1–M5 已闭环,A2A 机械层可演示。人说目标 → 墨墨写+自检 → 行首交闪闪 → 审批卡 → 人批准落地。顶栏显示球在谁手上;`@人` 可升级拍板;接力条可点开交接包。架构参考 clowder-ai,代码独立实现。和他们的差别:我们是**一场接力**,不是猫窝操作系统(不做邮箱/SOP/MCP)。
...
- 一条消息里多个 `@墨墨 @团团` = **同题并行**(每人收到同一份任务)。句中「不要 `@闪闪`」里的 `@` 也会被当成目标,演示时不要这样写
