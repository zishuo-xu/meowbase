...
>
> **现状**:M1–M6 已闭环。人说目标 → 墨墨写+自检 → 行首交闪闪 → 审批卡 → 人批准落地。忘了行首 `@` 且该交棒时,平台再问同一只一次。顶栏显示球在谁手上;`@人` 可升级拍板;接力条可点开交接包。说「之前约定」会召回已确认证据;整行 `星星罐子` 停棒。架构参考 clowder-ai,代码独立实现。和他们的差别:我们是**一场接力**,不是猫窝操作系统(不做邮箱/SOP/MCP)。演示与口播见 [docs/DEMO.md](docs/DEMO.md)。猫怎么交棒、传什么、各自记什么、公共记什么，见 [docs/A2A.md](docs/A2A.md)。功能一篇一刀，见 [docs/features/](docs/features/)。
