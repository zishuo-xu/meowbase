  ...
  if (input.profile) {
    ...
    const workdirRule = input.repo
      ? input.repo.allowRemote
        ...
        : `这是真实仓库 ${input.repo.path} 的 worktree,绝对路径 ${input.workdir ?? ''}。当前分支 ${input.repo.branch} 是你自己的,只在本地提交,不许推送、不许开 PR。不许碰 ${input.repo.baseBranch}、不许动 .git、不许切分支。所有文件创建/修改都发生在当前工作目录内,只使用相对路径,禁止读写工作目录以外的路径。`
