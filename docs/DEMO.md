...
给自己录 1–2 分钟、或给简历讲项目用。对照 clowder:他们闭环后放多猫演示;这一版演的是**一场接力**,邮箱/SOP 还没做,不在这次口播里。
...
绑仓推送(可选):侧栏填仓库路径,勾「允许推送和开 PR（会联网）」后新建会话。猫把 `meow/<id>` 推到 origin 时,时间线出「推到了 origin」,接力照跑。不勾则侧栏写「本地」,猫若仍推了,顶栏变成「球在人手里」。猫去推基准分支时无论勾没勾都停。人手合了 PR 后再一跳,那张还开着的审批卡变成已失效,没有批准/打回按钮。规则见协议表。

拉闸(可选):另起一行只写 `星星罐子`,顶栏变成「已拉闸,等人开口」,本轮不再叫猫。走偏时说 `脚手架` / `绕路了` / `喵约`,当轮注入对照技能。

Hub(可选):打开团队 Hub → 能力,应看见墨墨主架构师、闪闪审查官、团团执行者。只读名册,不改路由。

账本(可选):打开团队 Hub → 账本,切「当前线程 / 全部」。页上那句写明只算猫的、探测不计入。也可 `curl "localhost:3200/api/usage?threadId=t_xxx"`。今晚那条链:墨墨 输入 21,171 / 输出 1,936 / 缓存读 107,008 / 总计 130,115 / **$0.2078**;闪闪 输入 179 / 输出 557 / 缓存读 10,880 / 总计 13,465 / **无成本数据**;团团没出场,整行 `—`。「无成本数据」和「—」是两回事(跑过但没报邮资 vs 没出场)。

模型探测(可选):Hub → 模型目录,模型 ID 留空点「验证新模型」,应看见字段提示、没有探测中。填真模型名再点,结果里带输入/输出 token 和花费(没报成本则「无成本数据」)。
...
8. 先把接力做到可证明,再谈邮箱、SOP、MCP、队列;要做也不照搬他们那套。平台不推理是边界,不随节奏变。
