...
3. 期望:
   - 侧栏标题变成任务摘要(不是时间戳)
   - 墨墨改沙箱、回复里带本轮命令和结果
   - 行首 `@闪闪` → `🤝 接力:墨墨 → 闪闪`,顶栏球在闪闪;点「交接包」能看到目标和任务。墨墨改了文件才交棒,空信封不投递
   - 写完有改动却忘了行首 `@` → 墨墨会再开口一次补出口;仍没有才「球还在地上」
   - 行首 `@` 了但这一跳空手(没文件、没结论、正文很短) → 不传,顶栏「球还在地上」
   - 闪闪由平台接着跑,不必再发「继续」;写出「通过」后顶栏立刻「球在人手里」,不必等卡片刷出来;「需修改」则顶栏「球在写手手上」
   - 有改动时审批卡在闪闪跑完后出;侧栏该会话标 **待确认**;点进去滚到审批卡
4. 你点批准。期望顶栏变成 **已落地,等人开口**。

崩溃恢复(最值得演):第 3 步闪闪正在想、卡还没出来时下手——`lsof -ti :3200 | xargs kill -9`,再自己起一次 API(`tsx watch` 崩了不会重生)。不要发「继续」。今晚实测:22:22:23 杀进程,Redis 里那一棒还在(`pendingHop.id = eebe6ef7`),因为是**跑完落库才清**,不是读了就清;22:22:39 重启(隔 16 秒),日志依次 `resume sweep n=1` → `resume steal thread=6a920e0d` → `pending follow to=gemini` → `hop start agent=gemini`。期望:半截气泡标失败(「平台重启,这一跳没写完」),同一只猫自己接着跑完,审批卡自己出来。审计留下 `lease-steal` → `hop-failed` → `hop-rerun` → 22:23:32 `hop-done`,**全程同一个 hopId**。死者那把 60 秒租约还没过期,开机那一扫会强抢(单实例下没有活主人),所以立刻续跑而不是干等;重跑是真的重新推理——这次审查还带上了实测命令和结果,比第一次质量更高。代价:强抢假设单实例,多开一个 API 这条会破。

升级拍板(可选):让墨墨在两个方案里选一个。它行首写 `@人` 后,顶栏应是「球在人手里:墨墨请求拍板」,`💡` 提示不会盖住这句话。

持球(可选):墨墨行首写 `等 测试跑完` 后,顶栏「球在等:墨墨」,没有「球还在地上」,当轮不出审批卡。你再说一句,持球结束。行首 `等跑 npm test` 则由平台在沙箱跑测试,跑完再叫墨墨看结果。只认白名单里的形状(`npm test` / `git status` 那种);带分号、管道或 `node -e` 平台不跑,顶栏球回人手里。
...
审计(可选):`curl "localhost:3200/api/audit?threadId=t_xxx"`(倒序;可加 `actor` / `action` / `since` / `limit`)。期望按线程能回放谁开口、谁交给谁、哪张卡被建过、哪一跳抢到了租约。平台的决定在 store 边界自动落存根,业务代码里没有一处手写 `audit.append`,所以不存在「新写入点忘了记」;`lease-claim` 和后面那条 `hop-done` 是同一个 hopId,能证明这一跳走的是持久化那条路。今晚一条交棒链(正序,数字未改):
...
5. 人和猫都是行首 `@` 才点名/交棒,句中 `@` 不当球权;`@人` 升级给你;行首 `等` 持球。该交棒却没出口,平台再问同一只一次;行首 `@` 了但这一跳空手,平台不传,球回人。审查通过后顶栏球回人;需修改则球在写手手上。
