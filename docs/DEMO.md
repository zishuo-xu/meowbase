...
2. 只说目标,或另起一行点名。例如:
   ...
   不写 `@` 则走线程主猫(默认墨墨)。同题并行则 `@墨墨` 和 `@团团` 各占一行。
...
审计(可选):`curl "localhost:3200/api/audit?threadId=t_xxx"`(倒序;可加 `actor` / `action` / `since` / `limit`)。期望按线程能回放谁开口、谁交给谁、哪张卡被建过、哪一跳抢到了租约。平台的决定在 store 边界自动落存根;租约事件(`lease-claim` / `lease-steal`)和半截重跑(`hop-rerun`)仍显式补,见 [AGENTS.md](../AGENTS.md) 开发约定「审计不用手写」。`lease-claim` 和后面那条 `hop-done` 是同一个 hopId,能证明这一跳走的是持久化那条路。今晚一条交棒链(正序,数字未改):
