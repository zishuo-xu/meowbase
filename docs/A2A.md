...
**线程**  
每条会话自己的 `threadId`、消息列表、`work/<threadId>/`。线程 A 的文件和证据默认不进线程 B 的沙箱。证据召回的范围（按仓划界、空沙箱只看自己）见 [AGENTS.md](../AGENTS.md) 协议表，仍是只读片段，不是把旧线程整段对话贴过来。
...
我们：交棒后本轮先结束，下一跳进 pending，平台自己续跑。槽里已有棒时后来的交棒进队，当前棒跑完接着跑。人在忙时插的话进另一条队，不打断正在跑的猫。顶栏点开能看见后面排着谁、人还等着哪几句。交接包仍是 prompt 字符串。优先级、猫寄信却不交棒还没做。补问 / 空手不传见协议表。
