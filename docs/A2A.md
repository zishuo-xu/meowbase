...
5. 猫回复里再出现行首 `@闪闪 请审查`，平台停这只、记下 pending、插入 `🤝 接力`。本轮先结束，平台自己续跑下一只。`@人` 立刻停。行首 `等 原因` / `HOLD 原因` 持球，不补问、不掉地上、当轮不建卡。行首 `等跑` / `HOLDCMD` 同样持球，平台在沙箱跑命令，跑完再叫醒同一只。该交棒却没有行首 `@`（有 diff / 收了棒 / 句中 @ / 审查官没结论），平台再问同一只一次；仍没有才「球还在地上」。问答收尾、已写「通过 / 需修改」、持球不问。
