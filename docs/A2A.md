...
5. 猫回复里再出现行首 `@闪闪 请审查`，平台停这只、叫下一只，插入 `🤝 接力`（可点开交接包）。
6. 行首 `@人` / `@owner`：停链，球回人手里。
7. 已出场的猫不再回来（防环）。审查官写出「通过 / 需修改」后停棒。
8. 有文件 diff 时，平台建审批卡并拉审查——就算写手忘了 `@闪闪`。
...
我们：同一条 `executeTurn` 里同步走完一串棒；交接包是 prompt 字符串；隔离靠 session map + 沙箱；记忆靠证据库。要异步邮箱或「读对方 session」，就是另一套产品。
