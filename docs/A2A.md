...
5. 猫回复里再出现行首 `@闪闪 请审查`，平台停这只、记下 pending、插入 `🤝 接力`。本轮先结束，平台自己续跑下一只。**这一跳什么都没留下**（没新文件、没结论、去掉行首 `@` 后正文短于 60 字）则不传，球回人手里，不写 pending、不发接力句。`@人` 立刻停。行首 `等 原因` / `HOLD 原因` 持球，不补问、不掉地上、当轮不建卡。行首 `等跑` / `HOLDCMD` 同样持球，平台只跑白名单内的命令，跑完再叫醒同一只；不认得就不跑、球回人手里。该交棒却没有行首 `@`（有 diff / 收了棒 / 句中 @ / 审查官没结论），平台再问同一只一次；仍没有才「球还在地上」。问答收尾、已写「通过 / 需修改」、持球不问。长方案没改文件也可以交（fail-open）。
6. 行首 `@人` / `@owner`：停链，球回人手里。行首 `等` / `等跑`：猫还持球，人开口即取消。
...
我们：交棒后本轮先结束，下一跳进 pending，平台自己续跑。该交棒没出口则再问同一只一次。空手交棒不传，球回人手里。交接包仍是 prompt 字符串。不要邮箱。
