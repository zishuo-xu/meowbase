...
上一刀:**PR 上的 CI 变绿变红流回线程**(feat,已落地)。开着的 PR 检查绿只记时间线,红叫醒写手。下一刀候选是冲突检测,每条仍先写薄设计。
...
| 真能指着真仓库跑 | **中** | 能绑仓、能看见 git、能 push / 开 PR、越界或合了就停、PR 评论和 CI 红灯会流回线程；还没有冲突检测 |
...
- **留了没做**:冲突检测、自动 rebase、webhook 常驻轮询。
