...
上一刀：**空模型名不许假装连通，探测花费当场显示**（`platform-spend.md`，已落地）。模型名空着点验证会 400、适配器零调用；真探测把 usage 写在结果里（失败也显示）。账本口径写死为「只算猫的」，不新开 store。
...
| 真能指着真仓库跑 | **中** | 能绑仓、能看见 git、能 push 自己那根、越界就停；还不开 PR |
...
- **开 PR / 追 review** —— push 自己那根已经放开，越界闸也有了。下一层是他们的 F141 / F140（发现 + 追踪），单独一篇，不能顺手带。
...
- **还没验过（等人手跑）**：真模型名点「验证新模型」看结果里的 token 和花费——**这一步花钱，落地时没跑**。账本页那句「只算猫的」也只有单测，没对着浏览器看过。自动化覆盖的是纯逻辑那一半：空模型名 400、`createAdapter` 零调用（spy 断言）、成功和失败两条路径都透传 `usage`。
