...
上一刀:**MCP 配置可携带**(feat,已落地)。`GET /api/mcp/provision` 导出 claude 的 mcpServers 片段,Hub 能力页能看见。人自己贴到别的项目,平台不改别人的仓。下一刀候选从总账建议顺序里挑,每条仍先写薄设计。
...
| 方向 | 现在 | 说明 |
|---|---|---|
| 协议编排 | **中** | 路由、A2A 接力、持球、审查回合、交棒排队、人插话排队、提到前面都齐；拖拽改序和 urgent 自动置顶还没有，不能叫厚 |
