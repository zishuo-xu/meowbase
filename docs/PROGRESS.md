...
上一刀：**同一线程同一时刻只跑一跳**（`one-hop-per-thread.md`，已落地）。多 `@` 仍把同一正文发给每只点到的猫，但一只跑完再跑下一只；一条线程只有一个 `pendingHop` 槽，第一个交棒的留住球，后一个交棒落 `notice`。
...
| 方向 | 现在 | 说明 |
|---|---|---|
| 协议编排 | **中** | 路由、A2A 接力、持球、审查回合、技能注入都齐；但协议表里明写的多 `@` 直到这一刀才有同树互斥，不能再叫厚 |
