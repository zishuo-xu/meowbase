...
上一刀：**猫自己提交之后批准不再报提交失败**（`approve-after-self-commit.md`，已落地）。绑仓线程里猫自己 commit 之后，人点 `#approve` 走原来的「工作区干净且 HEAD 已越过 marker」兜底，卡记成已落地。根因是 git 的「没东西可提交」写在 stdout、而且输出跟着系统语言飘；这一刀钉 `LC_ALL=C` 并让 `gitErrorReason` 也读 stdout。
