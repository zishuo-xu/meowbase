...
上一刀:**等跑拒中文尾巴、补全换行不插候选**(fix,已落地)。`npm test 平台替跑确认…` 不再当前缀过白名单;补全开着时 Shift+Enter 只换行。下一刀候选从总账建议顺序里挑。
...
| 方向 | 现在 | 说明 |
|---|---|---|
| 协议编排 | **中** | 路由、A2A 接力、持球、审查回合、交棒排队、人插话排队、改序急件、跨轮乒乓和链尾收尾都齐；并行多 @ 仍是有意裁剪 |
| 可证明性 | **厚** | 六道闸进 CI，记分板把「兜住了」变成会红的数字，审计能回放 |
| 平台会不会拒绝 | **中** | 命令白名单 + 入口白名单 + 出仓开关 + 预算闸都有了；但**没有任何鉴权** |
| 真能指着真仓库跑 | **中** | 能绑仓、能看见 git、能 push / 开 PR、越界或合了就停；PR 评论、CI 红灯、合不进去都会流回线程。自动 rebase 没做 |
| 产品外壳 | **中** | Hub 仍只是只读能力表加名册配置；演示主路径的输入/渲染进了 CI，排队能在顶栏点开看见；Hub / 账本仍靠人手 |
| 记忆 | **中** | 按仓划界、注入带出处、Hub 能搜跨仓并标出处。池子仍只进不出，召回仍是 push 关键词，没有删除/编辑 |
| 技能 | **中** | 触发词当轮注入,Hub 能看见注入次数和工具调用;还不能判断说明书有没有被照着做,也没有按日趋势 |
...
### 2026-09-06 fix: 等跑拒中文尾巴、补全换行不插候选

白名单在前缀匹配之后检查多余参数:含中文的说明拒,`--` / flag 仍过。补全菜单开着时 Shift+Enter 关掉菜单并换行,不把高亮项插进正文。记分板加「等跑后面跟中文说明」。

- **偏离**:没做成「截掉尾巴只跑 npm test」,多余非 flag 一律拒,避免 npm 把人话当文件名。
- **只有人手验过**:没再对着真 npm 跑那串中文(单测和记分板盖了拒因)。
- **留了没做**:第二个 PR 消失、干活时顶栏不说哪只猫、Hub 默认页。

### 2026-09-06 feat: 出仓第二层审查

`second-layer-review.md`。`needsSecondLayerReview` 只认 `allowRemote === true`。开了远程就不走 autoApprove,卡文案写清等仓外第二层或人批。空沙箱仍可自动落地。

- **偏离**:没调 Codex,也不往 PR 评论写 @。第二层的投递面仍是已有 PR 回流和 `#approve`。
- **只有人手验过**:没对着真 GitHub review 看卡(单测盖了远程不落地、空沙箱仍落地)。
- **留了没做**:云端猫、按 GitHub review state 改卡、自动 @。

### 2026-09-06 feat: 记忆联邦检索

`memory-federation.md`。`searchEvidenceHits` 按关键词搜已确认证据,不要求「之前/约定」。跨仓命中标 `foreign`。Hub 记忆页能搜到 id,自动召回仍走划界。

- **偏离**:没做独立 Collection 表,一座仓就是一座馆;没做删除/编辑。
- **只有人手验过**:没对着两座真仓库在 Hub 里手搜(单测盖了纯函数、HTTP、Hub 展示)。
- **留了没做**:Collection CRUD、敏感级、生命周期、向量检索。

### 2026-09-06 feat: 跨轮乒乓熔断与收尾槽

`chain-quality.md`。`Thread.relayPairs` 记 `from>to` 次数;没文件没结论的同一对第三次空转 `stop=pingpong`。链尾再交棒 `stop=final-slot`,不再只落 notice。有文件或有结论的来回不拦。

- **偏离**:没做按内容打分的乒乓账本,也没改默认链深。
- **只有人手验过**:没拿真模型隔夜对打(单测盖了第三次拦、有文件不拦、maxDepth=1 收尾)。
- **留了没做**:内容评分、角色护栏、改链深默认值。

### 2026-09-06 feat: MCP 配置可携带

`mcp-provision.md`。`formatMcpProvision` 给出 claude / gemini / env 三块,形状对得上现有 `mcpCliArgs`。Hub 能力页展示 JSON。不往别的仓写文件,不引入 SDK。

- **偏离**:没做自动写入 ~/.claude,也没 npm 包装。
- **只有人手验过**:没对着另一份 claude 配置真贴(单测盖了形状、HTTP、Hub 展示)。
- **留了没做**:完整 SDK、资源模板、自动写入别人仓库。

### 2026-09-06 feat: 思考与计划分层

`thought-layers.md`。`splitThoughtLayers` 只从思考原文拆「计划:」/「Plan:」。跳完成写 `plan`;流式累积后再拆。气泡先计划后思考,默认收着。

- **偏离**:没开独立 plan WS,也没从正文抠计划。
- **只有人手验过**:没对着真模型看计划块(单测盖了拆层、落库、气泡、流式增量)。
- **留了没做**:独立遥测通道、固定 JSON 计划契约。

### 2026-09-06 feat: 按猫拆池与热改上限

`budget-pools.md`。`agentBudgets` 按猫封顶,只拦点名它的那一跳。`PATCH /api/config` 热改全平台和按猫上限,live 立刻用。Hub 账本两个输入。gemini 不报成本则该池花不满,不估。

- **偏离**:没做按日清零、按模型拆池。
- **只有人手验过**:没对着真模型改 Hub 再跑一跳(单测盖了按猫拒跑、落盘、账本表单)。
- **留了没做**:按日重置、估算 gemini、实时推送。

### 2026-09-06 feat: 排队改序与急件置顶

`queue-reorder.md`。`moveQueueItem` 把一条挪到指定位置;`POST /queue/steer` 可带 `beforeId`。人话 `!` / `急` 开头入队插队头并标急。面板上移/下移。不 abort 当前棒。

- **偏离**:没做鼠标拖拽手势,按钮改序够演示。两队仍分开。
- **只有人手验过**:没对着真模型插急件再看面板(单测盖了纯函数、入队置顶、上移回调、急标记)。
- **留了没做**:拖拽手势、两队合成一条、急件打断当前棒。

### 2026-09-06 feat: 群组 mention 展开

`group-mentions.md`。行首 `@all` / `@thread` / `@全员` / `@大家` 展开成名册全部;`@架构` / `@审查` / `@执行` 按角色。句中不算。补全菜单加「全员」。猫写群组名不交接。

- **偏离**:@thread 等于 @all,没做动态线程成员;品种按角色不是按 CLI。
- **只有人手验过**:没对着真模型打 @全员(单测盖了展开、剥名、整机三只顺序跑、补全插入、猫 @all 不交棒)。
- **留了没做**:并行群发、按 CLI 再拆一组、猫把棒交给 @all。

### 2026-09-06 feat: 记忆召回度量

`memory-recall.md`。跳完成记下 `evidenceIds`;引用扫正文 `#ev_`。`GET /api/usage/memory` 聚合注入/引用。Hub「记忆」页跟账本同一套当前线程/全部。空态不写 0。

- **偏离**:没做离线评测集和 nDCG,也不让猫打分。
- **只有人手验过**:没对着真模型翻 Hub 看数字(单测盖了聚合、HTTP、注入落库、Hub 列出)。
- **留了没做**:评测集、按日趋势、按仓拆表。

### 2026-09-06 feat: 跨线程传话带出处

`cross-thread-post.md`。`POST /api/threads/:id/cross-post` 把正文寄到目标线程,落 `cross-post` 系统消息,写明来自哪条。球权跳过。不落助手气泡,所以同名猫不会把自己在另一条线程说的话当成自己。

- **偏离**:没让猫经 MCP 自己寄,也不叫醒对方。
- **只有人手验过**:没对着真模型两条线程互寄(单测盖了文案、HTTP 400/404/200、球权跳过、Hub 一行)。
- **留了没做**:MCP cross_post、自动叫醒、把跨线程通讯变成审批。

### 2026-09-06 feat: 新会话带上已确认证据

`session-capsule.md`。`selectSessionCapsule` 只收 confirmed、新的在前、最多 8。无 `thread.sessions[agent]` 时并进 system prompt,标题标明续接胶囊不是本轮指令。有 session 不重复灌。

- **偏离**:没让旧猫写总结,也没任务板快照。
- **只有人手验过**:没对着真模型清 session 再开口(单测盖了挑选、标题、无 session 注入、有 session 不再灌)。
- **留了没做**:handoff.md、整条时间线、任务快照。
...
`approval-hub.md`。Hub 加「待批」页,`listApprovals()` 全量拉、只展示还没落地的卡。批准/打回发现有系统命令;去看关闭 Hub 并打开那条线程。空态「没有待批的卡」。

- **偏离**:没做推送和批量,也不另建提案表。
- **只有人手验过**:没对着真模型开两条线程再进 Hub 批(组件单测盖了列出、过滤已落地、批准和去看)。
- **留了没做**:推送、按功能过滤、把所有跨线程通讯变成审批。

### 2026-09-06 feat: 每一跳留一份原始记录

`hop-transcript.md`。适配器每读一行 stdout 调 `onRawLine`;有 hopId 才写入 `audit/hops/<threadId>/<hopId>.jsonl`。写失败不挡跳。`GET /api/hops/:hopId?threadId=` 读回。审计接口仍不含原文。

- **偏离**:没进 UI,没压 stderr。
- **只有人手验过**:没对着真模型翻一份 JSONL(单测盖了追加读回和 HTTP 404/200)。
- **留了没做**:时间线展示、按日压缩、进 Redis。

### 2026-09-06 feat: 确认了就写成文件

`memory-files.md`。`#confirm` 成功后写 `memory/ev_xxx.md`;开机 `rebuildEvidenceFromFiles` 灌回索引。文件是真相,Redis/内存是索引。写失败仍算确认,回执带「没写成文件」。

- **偏离**:没自动 git commit,也没向量库。
- **只有人手验过**:没对着真 Redis FLUSH 后再开机看重建(单测盖了确认写文件、清空后 rebuild)。
- **留了没做**:自动提交到仓、draft 落盘、Hub 编辑/删除。

### 2026-09-06 feat: 协作工具搜消息列线程

`collab-tools.md`。`searchMessages` / `listThreadIndex`;`GET /api/collab/messages` 与 `/api/collab/threads`;提示词告诉猫有这两把;Hub 能力页两行只读。空查询返回空。

- **偏离**:没起独立 MCP 进程,工具面先走 HTTP。
- **只有人手验过**:没让真模型去调这两把(单测和 HTTP 盖了空查询、关键词命中、列线程)。
- **留了没做**:stdio MCP 服务器、跨线程发信、三家 CLI 挂 MCP。

### 2026-09-06 feat: 家规写成告示牌

`sop-board.md`。`deriveSopBoard` 从球权和持球派生阶段;`Thread.sop` 跳前和 settle 后写入;提示词加「告示不是命令」一段;顶栏空闲以外显示 note。

- **偏离**:没做阶段机,也不让猫改牌子。
- **只有人手验过**:没对着真模型压缩后再看牌子(单测盖了派生、提示词注入、跳后落库、顶栏展示)。
- **留了没做**:人在 Hub 手改阶段、强制每步写阶段、压缩后恢复 CLI session。

### 2026-09-06 feat: 排队里提到前面

`queue-steer.md`。存储把指定人话 / 交棒挪到队头;`POST /api/threads/:id/queue/steer`;面板非队头显示「提到前面」。不 abort 当前棒,不碰槽里那一跳。

- **偏离**:没做拖拽和 urgent 标签,steer 只是挪到队头。
- **只有人手验过**:没对着真模型点开面板插队(单测和 HTTP 盖了挪队头、找不到 404、挂住的棒仍能拉闸结束)。
- **留了没做**:拖拽任意位置、urgent 自动置顶、立刻跑队里的条目。

### 2026-09-06 feat: 技能和工具用过几次

`skill-usage.md`。助手消息记下这一跳注入的 `skillIds`;工具次数扫已有 `activities`。`classifyTool` 分 builtin / skill / mcp,`思考` 不算。Hub 技能页和账本同一套当前线程 / 全部。接口 `GET /api/usage/tools`。

- **偏离**:没做 Redis 计数器和按日趋势,和账本一样打开再算。
- **只有人手验过**:没对着真模型开 Hub 技能页看一跳(单测和 HTTP 盖了注入落库、聚合、空态)。
- **留了没做**:按日趋势、JSONL 归档、判断技能有没有被照着做。

### 2026-09-06 feat: 花超了就拒跑

`budget-gate.md`。`isOverBudget` 纯函数;配了 `MEOW_BUDGET_USD` / `budgetUsd` 时,`executeTurn` 在系统命令之后、路由之前扫全平台已完成助手消息的真实花费,达到上限落 `budget`、不调猫。批准 / 拉闸 / 确认证据仍立刻走。Hub 账本显示上限。记分板加 `budget-gate`。

- **偏离**:无。没按猫拆池,没估 gemini。
- **只有人手验过**:没对着真模型把上限打到几美分再发第二句(单测和记分板用 fake 的 $0.001)。
- **留了没做**:按猫/按线程拆池、热改上限的 Hub 表单、按价格表估算。

### 2026-09-06 feat: PR 合不进去时流回线程

`pr-conflict.md`。绑仓开远程且 PR OPEN 时,每跳后 `gh pr view --json mergeable`。CONFLICTING 落 `pr-conflict` 并叫醒写手;变成 MERGEABLE 只记「冲突解开了」。UNKNOWN 不落。评论 / CI 叫醒已占槽则本轮不覆盖。记分板加 `pr-conflict`。

- **偏离**:无。没做自动 rebase。
- **只有人手验过**:没对真 GitHub 冲突 PR 验过(单测和记分板用 fake)。
- **留了没做**:自动 rebase / 自动解冲突、webhook 常驻轮询。

### 2026-09-06 feat: PR 上的 CI 变绿变红流回线程

`pr-ci-tracking.md`。绑仓开远程且 PR OPEN 时,每跳后 `gh pr checks`。PENDING 不落;SUCCESS 落 `pr-ci` 不叫醒;FAILURE/ERROR 落 `pr-ci` 且链已停则叫醒写手。指纹 `name:state` 投成功才记。评论叫醒已占槽则本轮不覆盖。记分板加 `pr-ci-red` / `pr-ci-green`。

- **偏离**:无。没做 intent=merge 时绿了去合。
- **只有人手验过**:没对真 GitHub Actions 验过 `gh pr checks`(单测和记分板用 fake)。
- **留了没做**:自动 rebase、webhook 常驻轮询。冲突检测已另篇。

### 2026-09-06 feat: 顶栏点开就能看见排队

`queue-panel.md`。新增 `QueuePanel`:空队不渲染;有货时顶栏球权行下面一条可点的「后面还有 N 棒 / 还有 N 句在等」,点开列出交棒队(谁交给谁 + 任务一句)和人话队(正文一句)。球权文案不再重复数量,避免两处各写一遍。切线程收起。`ThreadDto.pendingQueue` 补 `task`。

- **偏离**:无。没做拖拽改序。
- **只有人手验过**:没对着真模型开一条多 `@` 再插话点开面板看(组件单测盖了空/列出/收起/点开)。
- **留了没做**:拖拽、steer 插队、删队中某一条、跨线程总览。

### 2026-09-06 feat: 人插话先排队,不打断正在跑的猫

`user-message-queue.md`。`Thread` 加 `inboundQueue`;存储端口加 `enqueueInbound` / `shiftInbound` / `clearInboundQueue`。`POST /messages` 见忙(running / pendingHop / pendingQueue)则普通正文入队、回 202,不调 `executeTurn`。链跑完 `drainInbound` 取出队头再跑。星星罐子 / `#approve` / `#reject` / `#confirm` 立刻走;拉闸清人话队并 abort 当前棒。顶栏跟「还有 N 句在等」;输入框忙碌时仍可发,按钮写「排队」。

- **偏离**:无。拉闸顺手 abort 当前棒,否则挂住的请求结束不了,测试盖了这条。
- **只有人手验过**:没对着真模型在审查还没写完时插一句看顶栏「还有 1 句在等」(HTTP 集成盖了入队 202、拉闸清队、不打断直到拉闸才 abort)。
- **留了没做**:优先级、steer 插队、QueuePanel 拖拽、猫寄信却不交棒。

### 2026-09-06 feat: 交棒排队,跑完接下一条

`pending-handoff-queue.md`。`Thread` 加 `pendingQueue`;存储端口加 `enqueuePendingHop` / `promoteQueuedHop` / `clearPendingQueue`(memory + redis)。多 `@` 都交棒时第一个留槽、后一个入队,notice 改成「排在后面,当前棒跑完接着跑」。`followPendingChain` 槽空了就把队头填回去再跑;开机扫空槽但队非空也捡。星星罐子 / 越界 / PR 合了 / 人发了不续跑的新消息 / 等跑被拒:清槽也清队。顶栏 `describeBall` 加可选 `queuedCount`,同一行跟很淡的「后面还有 N 棒」。协议表、A2A、ALIGNMENT 第 6/29 行同轮改。

- **偏离**:无。人开口取消「等跑」也清队,因为那是新一轮,不是续跑。
- **只有人手验过**:没对着真模型开一条多 `@` 线程看顶栏「后面还有 1 棒」和两棒审查是否真的接上(单测盖了入队、出队续跑、拉闸清队、顶栏文案)。
- **留了没做**:人插话排队、优先级、steer 插队、队列面板、猫寄信却不交棒。

### 2026-09-06 fix: git 文件列表关 quotepath,中文文件名不再转义

多端协同压测时发现:沙箱里出现中文文件名后,交接包「改动文件」显示成 `"01-\350\256\276\350\256\241.md"`(git 默认 core.quotepath 的八进制转义)。修复:`gitChangedPaths` / `gitDiffHead` 里四处 `git diff` 调用加 `-c core.quotepath=false`。顺带修掉一个更深的连带:`gitDiffHead` 拿文件名当 pathspec 取 diff 正文,转义名匹配不到真实文件,非 ASCII 文件的改动会从审批卡 diff 里静默消失;一轮只改中文名文件时 `gitDiffHead` 整个返回 null、审批卡建不出来。测试「中文文件名原样出现在改动列表和审批 diff 里,不转义也不丢 diff」锁住(git.test.ts,TDD 先红后绿)。

- **偏离**:无(非设计稿功能,bug 修复)。曾考虑把 `-c` 加进 `run()` 帮手让所有 git 调用受益,那会让 `gitStatusPorcelain` / `parseStrayFiles`(清扫)和 `gitWorktreeList` 的输出一起变,超出本刀范围;收窄到只改产出用户可见文件列表的两处。
- **只有人手验过**:没在真模型线程重跑一次中文文件名的完整接力(单测在真 git 上盖住两个函数的真行为;现网复现现场是压测线程 `压测A-三棒接力` 的交接包)。
- **留了没做**:`gitStatusPorcelain` 的 porcelain 输出仍被 quotepath 转义(影响:清扫认不出中文名散落文件,行为偏保守,没观察到实际咬人);`gitWorktreeList` 同理。哪天动清扫再一起说。
