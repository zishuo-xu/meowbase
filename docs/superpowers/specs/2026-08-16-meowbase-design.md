...
> **这是 2026-08-16 的历史总图,别当现行能力表。** 之后还做了持球、等跑、虚空门禁、记分板等。现行语义以 [AGENTS.md](../../../AGENTS.md) 协议表和 [docs/features/](../../features/) 为准。正文不改写。

- 日期:2026-08-16
- 状态:已批准(用户确认开工);**M1–M6 已落地**。A2A 机械层可演示;证据可按「之前约定」召回;人可整行 `星星罐子` 拉闸。演示与口播见 [docs/DEMO.md](../../DEMO.md)。协作怎么传信息、怎么隔离上下文,见 [docs/A2A.md](../../A2A.md)。现行功能为什么这样设计,见 [docs/features/](../../features/)
