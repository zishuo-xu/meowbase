...
架构参考 [clowder-ai](https://github.com/zts212653/clowder-ai)（MIT）的公开设计，代码独立实现。差别是形态和进度：他们已经长成猫窝操作系统（邮箱、SOP、MCP）。喵窝先做**一场接力**：一只做完，平台把棒交给下一只。
...
- 演示主路径的输入/渲染由 `pnpm e2e:web` 走。Hub / 持球 / 崩溃在浏览器里怎么长，仍是人手。WS 推 `sync` 有集成测试（`http-live-sync.test.ts`），前端收到 `sync` 会重拉有 jsdom 单测（`page.test.tsx`）。
...
**先不做，不是不做。** 邮箱、SOP 手册、MCP 规格都还没有。顺序是刻意的：先把一场接力做到**可证明**（六道闸、记分板、审计能回放），再往上加形态；反过来先铺形态，坏了都不知道是哪一层坏的。要做也不照搬他们那套，得先想清楚在接力这个形态里它该长什么样。

**只有一条不随节奏变：平台不推理**（开头那段说了是哪三件事）。那是边界，不是待办。

**现在还薄。** 线程能绑真实仓库，改动落在 `meow/<threadId>` worktree，猫可以 push 自己这根、对自己这根开 PR；碰基准分支或把 PR 合进去则停接力（事后闸，合已经发生）。不做 webhook / review 回流。账本能按猫看 token 和花费（`GET /api/usage`），但只能看、**不能按预算拦**。Hub 是只读能力表加名册配置，不是完整产品外壳。
