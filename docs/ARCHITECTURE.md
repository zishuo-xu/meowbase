...
**现在还薄。** 线程能绑真实仓库，改动落在 `meow/<threadId>` worktree，猫可以 push 自己这根；碰基准分支则停接力（事后闸，那一 push 已经发生）。不开 PR。账本能按猫看 token 和花费（`GET /api/usage`），但只能看、**不能按预算拦**。Hub 是只读能力表加名册配置，不是完整产品外壳。
