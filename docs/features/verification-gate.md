...
- **功能**：审查官写「通过」但没有本轮命令+结果时，卡上 `verdict` 变成 `incomplete`，不许 `autoApprove`。不管顶栏文案（顶栏读审查正文关键词）。边界见 [AGENTS.md](../../AGENTS.md) 协议表
- **价值**：人不用猜猫有没有真跑过
- **愿景**：邮差查证据字段，不替猫跑测试
...
- **30 秒**：闸只管卡上结论和自动落地，不管顶栏。没证据时卡上写「结论不算通过」，人仍该拍板；顶栏读的是审查正文里的「通过」。
